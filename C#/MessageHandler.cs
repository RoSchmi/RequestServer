...
namespace ArkeIndustries.RequestServer {
    ...
    public sealed class MessageParameterAttribute : Attribute {
        ...
        public MessageParameterAttribute(long index) => this.Index = index;
    }
    ...
    public abstract class MessageHandler {
        ...
        internal static List<PropertyInfo> GetProperties(Type type) => type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                .Where(p => p.IsDefined(typeof(MessageParameterAttribute)))
                .OrderBy(p => p.GetCustomAttribute<MessageParameterAttribute>().Index)
                .ToList();

        internal List<ParameterNode> CreateTree(Type type) => MessageHandler.GetProperties(type).Select(p => this.CreateTree(p)).ToList();
